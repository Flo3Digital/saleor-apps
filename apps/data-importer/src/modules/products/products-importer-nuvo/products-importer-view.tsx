...
import {
  ProductColumnSchema,
  getProductsModelColumns,
  getResultModelSchema,
} from "./products-columns-model";
...
import { ProductsImportingResults } from "../products-results/products-importing-results";
...
const columns = getProductsModelColumns();
...
const getNuvoSettings = (theme: SaleorTheme): SettingsAPI => {
  ...
  return {
    ...
    title: "Upload products to Saleor",
...
export const ProductsImporterView = () => {
  const authorized = useAuthorizedToken("MANAGE_PRODUCTS");
  const saleorTheme = useTheme();

  const [importedLines, setImportedLines] = useState<ProductColumnSchema[] | null>(null);
  ...
  if (authorized === false) {
    return <Alert variant="error">To use this importer you need MANAGER_PRODUCTS permission</Alert>;
  }

  if (importedLines) {
    return <ProductsImportingResults importedLines={importedLines} />;
  }

...
const addDeliveryNotePages = async (input: {
  order: OrderPayloadFragment;
  pdfDoc: PDFDocument;
}): Promise<{ pdfDoc: PDFDocument }> => {
  const { order, pdfDoc } = input;
  ...
  page.drawText("DELIVERY NOTE", secondSectionFirstColumn({ size: fontSize.md }));
  page.drawText(deliveryNoteString, secondSectionSecondColumn({ size: fontSize.md }));
...
to Kowloon HK$200 & New Territories HK$350)`,
  ...
  const totalPagesForAllItems =
    totalPagesFor25Items === 1 && totalItems > 15 ? totalPagesFor25Items + 1 : totalPagesFor25Items;
  ...
  for (let pageNumber = 0; pageNumber < totalPagesForAllItems; pageNumber++) {
    ...
    if (pageNumber + 1 === totalPagesFor25Items) {
      ...
      if (orderFromQuery?.shippingMethodName && orderFromQuery.shippingPrice?.gross?.amount) {
        const tableRowArray = [orderFromQuery.shippingMethodName, "-", "-", "-", "-", "-", "N/A"];
      ...
      if (
        ...
        const paymentSectionDetail: { name: string; value: string }[] = [
          ...
          { name: "Bank address:", value: `N/A` },
          { name: "SWIFT code:", value: `N/A` },
          { name: "Account name:", value: `N/A` },
          {
            name: "Account number:",
            value: `N/A`,
          },
          { name: "FPS ID:", value: `N/A` },
        ];
        ...
        pageForDraw.drawText(
          `Remark`,
          secondSectionFirstColumn({ y: height - remarkSectionHeight, size: fontSize.md })
        );

        const remarkTest = orderFromQuery?.metafield || "N/A";

        pageForDraw.drawText(
          remarkTest,
          secondSectionFirstColumn({ y: height - (remarkSectionHeight + 20) })
        );

        const footerSectionHeightForBottom = remarkSectionHeight + 100;
        ...
        paymentSectionDetail.forEach((each, index) => {
          ...
          page2.drawText(each.name, secondSectionFirstColumn({ y: height - currentSectionHeight }));
          page2.drawText(
            each.value,
            secondSectionSecondColumn({ y: height - currentSectionHeight })
          );
...
const addTestDeliveryNotePages = async (input: {
  pdfDoc: PDFDocument;
}): Promise<{ pdfDoc: PDFDocument }> => {
  const { pdfDoc } = input;
...
export { addDeliveryNotePages, addTestDeliveryNotePages };
